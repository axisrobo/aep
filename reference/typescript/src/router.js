...
export class EventRouter {
      ...
      if (match(event)) {
        ...
        if (responseOrResponses !== undefined && responseOrResponses !== null) {
          if (Array.isArray(responseOrResponses)) {
            results.push(...responseOrResponses);
          } else {
            results.push(responseOrResponses);
          }
        }
  ...
  static pattern(pattern) {
    return {
      forSession: (sessionId) => (event) =>
        matchesType(pattern, event.type) && event.session_id === sessionId
    };
  }
}

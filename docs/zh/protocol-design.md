...
工作名称：**AEP — Agent Event Protocol（智能体事件协议）**。
...
- `delegation.requested` -- 请求委托
- `delegation.accepted` -- 委托已接受
- `delegation.rejected` -- 委托被拒绝
...
AEP 应避免承诺严格的恰好一次交付。相反，每个事件都有一个稳定的 ID 以便消费者可以去重。

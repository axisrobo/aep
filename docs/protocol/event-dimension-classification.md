...
| `interruption.*`, `compensation.*` | Recovery | Module not yet created; stays hardcoded in legacy registries |
...
**Recovery dimension** (`interruption.*`, `compensation.*`): Interruption lifecycle and compensation semantics belong to the Resilience/Recovery dimension. A dedicated module will own these when created. Until then they remain hardcoded in legacy registries.

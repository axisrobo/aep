...
- `InMemoryDeliveryStore` — volatile, process-local.
- `SqliteDeliveryStore` — embedded, file- or memory-backed.
...
|---|---|---|
| TypeScript | `pg` | add to `reference/typescript/package.json` |
...
| Java | `org.postgresql:postgresql` JDBC | add to `reference/java/pom.xml` |
...
- Coverage mirrors the SQLite test suite: track→ack, track→nack (attempt increment), track→dead-letter, `getPendingForSubscription` filtering, `hasAttemptsRemaining`, and `getStats`.
...
- TypeScript: `cd reference/typescript && npm test`
- Python: `cd reference/python && python -m pytest`
- Go: `cd reference/go && go test ./...`
- Java: `cd reference/java && mvn test`

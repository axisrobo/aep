...
- Create a top-level `examples/` directory with two categories:
  - `examples/sdk/` — embed the SDK via `import { ... } from "@axisrobo/aep"`.
  - `examples/service/` — connect to a running `aepd` over WebSocket and the HTTP API.
...
- `agent-subscriber.js` — migrate to `examples/sdk/agent-subscriber.js`, import `@axisrobo/aep`.
- `memory-event-producer.js` — migrate to `examples/sdk/memory-event-producer.js`, import `@axisrobo/aep`.
- `async-tool-producer.js` — migrate to `examples/sdk/` if it only uses public exports; otherwise keep under reference and note as internal.
- `mcp-aep-consumer.js` and `mcp-bridge/demo.js` — migrate to `examples/sdk/` if they use only public exports; otherwise keep as reference internals.
- `production-e2e.js` — keep as a reference internal test harness; it is not a consumer example.
- `runtime-service/README.md` — fold into `examples/README.md`.

...
function matchesValue(expected, actual) {
  return expected === undefined || toArray(expected).includes(actual);
}
...
function matchDottedPattern(pattern, value) {
  ...
  return patternParts.length === valueParts.length && patternParts.every((part, index) => part === "*" || part === valueParts[index]);
}

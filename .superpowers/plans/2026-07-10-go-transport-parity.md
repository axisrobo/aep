...
- [ ] `go test ./aep/ -run TestStdio -v` → PASS, full suite ~53 pass
...
- [ ] `go test ./aep/ -run TestSse -v` → PASS, full suite ~56 pass
...
- [ ] `go test ./aep/ -run TestWs -v` → PASS, full suite ~60 pass
- [ ] Commit: `feat: add Go WebSocket transport`

### Task 4: Docs, Verify, Push

- [ ] Update `reference/go/README.md` scope — list all 4 transports

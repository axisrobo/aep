...
AEP 被设计为 MCP 的异步对等协议。MCP 擅长同步能力调用：列出工具、调用工具、读取资源并返回即时结果。AEP 专注于 MCP 天然不覆盖的通信模式：事件流、长时间运行的任务生命周期、后台反馈、记忆更新、上下文失效、持久化交付、重放、取消和智能体间协调。

## 关于

AEP 0.1 草案是一个多语言协议仓库，包含：

- **16 个协议规范** 覆盖会话、订阅、任务、错误、版本控制、交付、可靠性、安全、一致性和传输层
- **4 个参考实现**（TypeScript, Python, Go, Java），基于共享夹具验证了跨语言一致性
- **4 个传输绑定**（stdio, WebSocket, SSE, gRPC）在所有语言中实现
- **SQLite 支持的交付存储** 在每个语言中提供重试、死信和重放支持
- **约 320 个测试** 跨四种语言全部通过
- **规范站点** 位于 [axisrobo.github.io/aep](https://axisrobo.github.io/aep/)
...
AEP 应与 MCP 互操作而非分叉。AEP 可以传递有关 MCP 工具调用的事件，但应保持协议独立性，以支持非 MCP 的智能体、工具、记忆系统、机器人系统、浏览器、IDE 和云运行时。
...
- `docs/specs/conformance.md` — 一致性级别和共享夹具清单
- `docs/specs/event-registry-governance.md` — 事件类型注册表治理和版本控制
- `CONTRIBUTING.md` — 贡献指南和仓库约定
- `CODE_OF_CONDUCT.md` — 贡献者行为准则

## 仓库布局

- `docs/` — 协议愿景、架构、设计草稿、规范、路线图和生成站点
- `docs/specs/` — 按层组织的协议规范
- `schemas/` — 共享 JSON Schema 资产
- `conformance/` — 共享一致性清单和夹具
- `reference/` — 语言特定的参考实现
- `reference/typescript/` — 主要可运行的 AEP 参考实现
- `reference/python/` — Python 参考实现，含完整传输和交付支持
- `reference/go/` — Go 参考实现，含 C0/C1 一致性
- `reference/java/` — Java 参考实现，含 C0/C1 一致性（JDK 21）
- `.github/workflows/` — 仓库 CI
- `.superpowers/` — Superpowers 开发工具工件
- `tools/` — 开发工具（一致性运行器、站点生成器）
...
此命令在所有四种语言参考实现上运行共享夹具，并打印统一的通过/失败矩阵。
...
AEP 是一个草案开放协议，拥有四个活跃的参考实现（TypeScript、Python、Go、Java），保持跨语言一致性。仓库包含按层组织的会话、订阅、任务生命周期、错误模型、版本控制、一致性、交付、可靠性、安全、事件注册表治理规范，以及四个传输绑定（stdio、WebSocket、SSE、gRPC）。每个参考实现支持 SQLite 支持的交付存储，跨语言一致性运行器（`node tools/conformance-runner.js`）在全部四种语言上验证共享夹具。
